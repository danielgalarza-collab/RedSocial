...
class PhotoFeedAPI(ListAPIView):
    ...
    #permission_classes = [IsAuthenticated] #-->para que pida autenticacion
    #serializer_class = PhotoSerializer

    #para que no pida autenticacion
    ...
    def get_queryset(self):
        ...
        # Si NO está autenticado → feed público
        if user.is_anonymous:
            return Photo.objects.all().order_by("-created_at")

        # Si está autenticado → feed de gente que sigue
